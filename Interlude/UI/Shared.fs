...
module Globals =
    ...
    module Style =
        ...
        let white : ColorFunc = K Color.White
        let black : ColorFunc = K Color.Black

        /// 1.0, 0.0
        let highlight : ColorFunc = fun () ->
            accentShade(255, 1.0f, 0.0f)
            
        /// 1.0, 0.5
        let highlightL : ColorFunc = fun () ->
            accentShade(255, 1.0f, 0.5f)

        /// 0.25, 0.0
        let highlightD : ColorFunc = fun () ->
            accentShade(255, 0.25f, 0.0f)
        
        /// 0.25, 0.5
        let main : ColorFunc = fun () ->
            accentShade(255, 0.25f, 0.5f)
        
        /// 0.25, 0.75
        let mainL : ColorFunc = fun () ->
            accentShade(255, 0.25f, 0.75f)
        
        /// 0.1, 0.25
        let mainD : ColorFunc = fun () ->
            accentShade(255, 0.1f, 0.25f)