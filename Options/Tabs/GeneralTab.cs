...
namespace YAVSRG.Options.Tabs
{
    class GeneralTab : Widget
    {
        public GeneralTab()
        {
            ...
            AddChild(new FramedButton("buttonbase", "Change profile", () => { Screen.AddDialog(new ProfileDialog((s) => { })); })
                .PositionTopLeft(-350, 550, AnchorType.CENTER, AnchorType.MIN).PositionBottomRight(-50, 625, AnchorType.CENTER, AnchorType.MIN));
            AddChild(new FramedButton("buttonbase", "Rename profile", () => { Screen.AddDialog(new TextDialog("New Profile Name:", (s) => { Game.Options.Profile.Name = s; })); })
