...
type private Leaderboard() =
    ...
    override this.Init(parent) =
        ...
        |+ Conditional(
            ...
            EmptyState(Icons.connected, %"misc.server_error")
        )
...
type private CompareFriend
    (
        ruleset: Ruleset,
        data_by_level: (Level * (TableChart * (int * float) option) array) array,
        name: string,
        on_back: unit -> unit
    ) =
    inherit StaticContainer(NodeType.None)
                            ...
                            for level, your_scores in data_by_level do
                                ...
                                for chart, your_score in your_scores do
                                    ...
                                    let name =
                                        match
                                            Cache.by_key
                                                (sprintf "%s/%s" (Table.current().Value.Name) chart.Hash)
                                                Library.cache
                                        with
                                        | Some cc -> cc.Title
                                        | None ->
...
type private FriendComparer(ruleset: Ruleset, score_data: (Level * TableChart * int option * float option) array) =
    ...
    let data_by_level =
        ...
        |> Array.map (fun (l, data) ->
            (l,
             data
             |> Array.map (fun (_, chart, grade, _) ->
                 if grade.IsSome then
                     chart, Some(grade.Value, score_of chart.Hash)
                 else
                     chart, None
             ))
        )
    ...
    override this.Init(parent) =
        ...
        |+ Conditional(
            ...
            EmptyState(
                Icons.multiplayer,
                %"stats.table.friends.empty",
                Subtitle = %"stats.table.friends.empty.subtitle"
            )
        )
        |* swap
...
type private TableLevelStats(level: Level, data: (int * int) array, ruleset: Ruleset, scale: float32) =
    ...
    override this.Draw() =
        ...
        Text.fill_b (Style.font, level.Name, b.SliceLeft(150.0f), Colors.text_subheading, Alignment.CENTER)
...
type private TableStats() =
    ...
    override this.Init(parent) =
        ...
        | Some t ->
            ...
            | Some ruleset ->
                ...
                |+ (GridFlowContainer(
                    ...
                    |+ (button (sprintf "%s %s" Icons.stats (%"stats.table.breakdown"), table_breakdown)
                        |> fun b ->
                            b.TiltLeft <- false
                            b)
                    ...
                    |+ (button (
                            sprintf "%s %s" Icons.multiplayer (%"stats.table.friends"),
        ...
        | None -> this |* EmptyState(Icons.table, %"stats.table.no_table")
