...
type RotationPicker(rotation: Setting<float>) as this =
    ...
    let fd() = Setting.app (fun x -> (x + 22.5) % 360.0) rotation; Style.click.Play()
    let bk() = Setting.app (fun x -> (x - 22.5) %% 360.0) rotation; Style.click.Play()
    ...
    do 
        ...
        |* Clickable((fun () -> (if not this.Selected then this.Select()); fd ()),
            OnRightClick = (fun () -> (if not this.Selected then this.Select()); bk ()),
    ...
    override this.Draw() =
        ...
        Draw.quad (Quad.ofRect this.Bounds |> Quad.rotateDeg rotation.Value) (Quad.colorOf Color.White) (Sprite.gridUV (3, 0) sprite)
        base.Draw()
...
type RotationSettingsPage() as this =
    ...
    let _rotations, refreshRotations =
        refreshRow
            ...
            (fun i k ->
                ...
                let n = float32 i
                RotationPicker(g keycount.Value i,
    ...
    do
        this.Content(
            column()
            |+ PageSetting("noteskins.edit.userotation",
                    Selector<_>.FromBool(use_rotation) )
                .Pos(200.0f)
                .Tooltip(Tooltip.Info("noteskins.edit.userotation"))
            |+ PageSetting("generic.keymode",
                    Selector<Keymode>.FromEnum(keycount |> Setting.trigger (ignore >> refreshRotations)) )
                .Pos(270.0f)
