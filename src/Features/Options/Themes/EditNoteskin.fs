    ...
    do
        this.Content(
            ...
            |+ PrettySetting("themes.editnoteskin.columnwidth", Slider(columnWidth, 0.05f))
                .Pos(310.0f)
                .Tooltip(Tooltip.Info("options.themes.editnoteskin.columnwidth"))
            |+ PrettySetting("themes.editnoteskin.columnspacing",Slider(columnSpacing, 0.05f))
                .Pos(390.0f)
                .Tooltip(Tooltip.Info("options.themes.editnoteskin.columnspacing"))
            ...
            |+ PrettySetting("themes.editnoteskin.globalcolors",
                ...
                .Pos(560.0f)
                .Tooltip(Tooltip.Info("options.themes.editnoteskin.globalcolors"))
            |+ PrettySetting("themes.editnoteskin.colorstyle",
                    Selector.FromEnum(
                        ...
                        |> Setting.trigger (ignore >> refreshColors)) )
                .Pos(630.0f)
                .Tooltip(Tooltip.Info("options.themes.editnoteskin.colorstyle"))
            |+ PrettySetting("themes.editnoteskin.notecolors", colors)
                .Pos(700.0f, Viewport.vwidth - 200.0f, 120.0f)
                .Tooltip(Tooltip.Info("options.themes.editnoteskin.notecolors"))
            |+ PrettyButton.Once("themes.editnoteskin.export", Noteskins.exportCurrent)
                .Pos(820.0f)
                .Tooltip(Tooltip.Info("options.themes.editnoteskin.export"))
        )
