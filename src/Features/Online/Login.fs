...
type RegisterPage(discord_tag) as this =
    ...
    do
        this.Content(
            column()
            |+ PageSetting("register.username", TextEntry(username, "none")).Pos(250.0f)
            ...
            |+ PageButton("register.terms_of_service", fun () -> openUrl("https://yavsrg.net/terms_of_service.md")).Pos(650.0f)
            |+ PageButton("register.privacy_policy", fun () -> openUrl("https://yavsrg.net/privacy_policy.md")).Pos(720.0f)
