...
type InviteCard(sender: string, lobby: System.Guid) =
    ...
    override this.Init(parent) =
        this
        |+ Text(Icons.MAIL + " " + sender, Position = Position.Margin(5.0f), Align = Alignment.LEFT)
        |+ Button(
            Icons.CHECK,
            (fun () ->
...
type InviteList() =
    ...
    do
        Network.Events.receive_invite.Add(fun (name, id) -> container.Add(InviteCard(name, id))
        //if
