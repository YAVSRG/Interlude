...
type NoteskinCard(data: RepoEntry) as this =
    ...
    let mutable status =
        if
            Noteskins.list ()
            |> Seq.map snd
            |> Seq.map (fun ns -> ns.Config.Name)
            |> Seq.contains data.Name
        then
            Installed
        else
            NotDownloaded
    ...
    member this.LoadPreview(img: Bitmap) =
        preview <- Some <| Sprite.upload_one true (SpriteUpload.OfImage("NOTESKIN_PREVIEW", img))
    ...
    type NoteskinSearch() as this =
        ...
        override this.Init(parent) =
            WebServices.download_json (
                "https://raw.githubusercontent.com/YAVSRG/Interlude.Noteskins/main/index.json",
                fun data ->
                    match data with
            ...
            |+ (SearchBox(
                Setting.simple "",
                (fun (f: Filter) -> grid.Filter <- NoteskinCard.Filter f),
